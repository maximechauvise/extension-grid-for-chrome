...
async function renderList() {
  ...
  if (layouts.length === 0) {
    container.innerHTML = '<p class="empty-state">Aucun layout. Commence par en créer un !</p>';
    return;
  }
  ...
  layouts.forEach((layout) => {
    ...
    item.innerHTML = `
      ...
      <span class="layout-item-preset">${layout.preset}</span>
    ...
    item.querySelector('.btn-delete').addEventListener('click', async () => {
      if (confirm(`Supprimer le layout "${layout.name}" ?`)) {
...
function buildProportionEditor() {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      cell.style.width = colPcts[c] + '%';
      cell.style.height = rowPcts[r] + '%';
      cell.textContent = CELL_LETTERS[r * cols + c];
...
function buildPreviewSVG(preset, size = 56) {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      rects += `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${cw.toFixed(1)}" height="${rh.toFixed(1)}" rx="2" fill="#45475a"/>`;
    }
  }
...
document.getElementById('import-file').addEventListener('change', async (e) => {
  ...
  try {
    ...
    alert(`${imported.length} layout(s) importé(s).`);
    renderList();
  } catch (err) {
